...
//Início do seu código
function divisiveisPorOitoDecrecente(max, min){
...
function resolveEquacaoProvaA(vetor: number[]): number[] {
  ...
  for (let i = 0; i < vetor.length; i++) {
    let x = vetor[i]
    let resultado = (x**3+2*x**2+6*x+5) / (x**2+2*x+1)
...
function resolveEquacaoProvaB(vetor: number[]): number[] {
  ...
  for (let i = 0; i < vetor.length; i++) {
    let x = vetor[i]
    let resultado = (x**3+2*x**2+6*x+5) / (x**3+2*x**2+1)
    resultados.push(resultado)
  }
  return resultados
}
...
function encontrarLivrosPorCategoria(livros: LivroCategoria[], categoria: string): LivroCategoria[] {
  let livrosEncontrados: LivroCategoria[] = []
  for (let i = 0; i < livros.length; i++) {
    if (livros[i].categoria == categoria) {
      livrosEncontrados.push(livros[i])
    }
  }
...
// Início do seu código
function aumentarPreco(items, factor: number) {
  let novosPrecos: number[] = []
  for (let i = 0; i < items.length; i++) {
    let novoPreco = items[i].preco * factor
...
//Início do seu código
function divisiveisPorTresDecrecente(max, min){
  let resultado:number[] = []
  for (let i= max; i >= min; i--){
    if(i%3==0 && i>10){
      resultado.push(i)
